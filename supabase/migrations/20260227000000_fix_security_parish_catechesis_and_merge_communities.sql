-- ==============================================
-- FIX SECURITY: views, parish_catechesis RLS, merge_communities role check
-- ==============================================
-- 1. Re-apply security_invoker on views (lost when recreated in dioceses migration)
-- 2. Replace parish_catechesis policies with role-based access control
-- 3. Add contributor/admin check to merge_communities function
-- ==============================================

-- ==============================================
-- 1. FIX VIEWS - Re-apply security_invoker = true
-- ==============================================
-- The dioceses migration (20260214) recreated these views but did not
-- re-apply security_invoker, reverting them to security_definer (default).
-- Security definer views bypass RLS, which is a security risk.

ALTER VIEW public.view_communities_by_parish SET (security_invoker = true);
ALTER VIEW public.view_priests_report SET (security_invoker = true);
ALTER VIEW public.view_catechist_teams SET (security_invoker = true);

-- ==============================================
-- 1. FIX parish_catechesis RLS POLICIES
-- ==============================================

-- Drop existing permissive policies
DROP POLICY IF EXISTS "Enable read access for all users" ON public.parish_catechesis;
DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON public.parish_catechesis;
DROP POLICY IF EXISTS "Enable update for authenticated users only" ON public.parish_catechesis;
DROP POLICY IF EXISTS "Enable delete for authenticated users only" ON public.parish_catechesis;

-- SELECT: All authenticated users can read
CREATE POLICY "parish_catechesis_select_authenticated" ON public.parish_catechesis
  FOR SELECT
  TO authenticated
  USING (true);

-- INSERT: Only contributors and admins
CREATE POLICY "parish_catechesis_insert_contributor_admin" ON public.parish_catechesis
  FOR INSERT
  TO authenticated
  WITH CHECK (public.is_contributor_or_admin());

-- UPDATE: Only contributors and admins
CREATE POLICY "parish_catechesis_update_contributor_admin" ON public.parish_catechesis
  FOR UPDATE
  TO authenticated
...
-- DELETE: Only admins
CREATE POLICY "parish_catechesis_delete_admin" ON public.parish_catechesis
  FOR DELETE
  TO authenticated
  USING (public.is_admin());

-- ==============================================
-- 2. FIX merge_communities FUNCTION - Add role check
-- ==============================================
...
BEGIN
  -- Role check: only contributors and admins can merge communities
  IF NOT public.is_contributor_or_admin() THEN
    RAISE EXCEPTION 'Permiso denegado: solo contributors y admins pueden fusionar comunidades';
  END IF;
